...
async fn merger_node(
    ...
    if has_a && has_b {
        ...
        eprintln!(
            "[merger] WARNING: only some workers visible — join may not be working correctly"
        );
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_worker_b = final_log
        .iter()
        .any(|v| v.as_str().map(|s| s.contains("worker_b")).unwrap_or(false));
    let has_merger = final_log
        .iter()
        .any(|v| v.as_str().map(|s| s.contains("merger")).unwrap_or(false));
    let has_output = final_log
        .iter()
        .any(|v| v.as_str().map(|s| s.contains("output")).unwrap_or(false));

    println!("Assertions:");
    assert!(has_entry, "FAIL: entry missing");
    println!("  ✅ entry ran");
    assert!(has_worker_a, "FAIL: worker_a missing");
    println!("  ✅ worker_a ran");
    assert!(has_worker_b, "FAIL: worker_b missing");
    ...
    assert!(has_output, "FAIL: output missing");
    println!("  ✅ output ran");
    ...
    .filter_map(|needle| {
        final_log
            .iter()
            .position(|v| v.as_str().map(|s| s == *needle).unwrap_or(false))
    })
    .collect();
    ...
    if positions.len() == 5 {
        ...
        assert!(
            ...
            "FAIL: merger ran before workers completed! positions={:?}",
            positions
        );
